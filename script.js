/* Wadhwani Admin Dashboard V6 — controlled facilitator refresh */
(() => {
  'use strict';
  ...
  async function runStudentLiveSync() {
    studentLiveTimer = null;
    if ($('#studentHomeScreen')?.classList.contains('d-none') || !currentStudent) return;
    ...
    try {
      ...
      if (currentStudent && !$('#studentHomeScreen')?.classList.contains('d-none')) scheduleStudentLiveSync();
    }
  }
  ...
  function handleLiveSyncVisibility() {
    ...
    if (!$('#studentHomeScreen')?.classList.contains('d-none') && currentStudent) scheduleStudentLiveSync(500);
  ...
  function initializeMessengerBrowserFeature() {
    ...
    const continueButton = $('#browserContinueButton');
    ...
    if (instructionScreen.dataset.browserFeatureBound !== 'true') {
      ...
      continueButton.addEventListener('click', showMainApplication);
  ...
  function bindEvents() {
    ...
    $('#college').addEventListener('change', populateCourses);
  ...
  function populateColleges() {
    ...
    if (existing && COLLEGES[existing]) select.value = existing;
    populateCourses();
  }
  ...
  function populateCourses() {
    ...
    $('#course').disabled = !college;
    (COLLEGES[college] || []).forEach(course => $('#course').add(new Option(course, course)));
    if ((COLLEGES[college] || []).includes(currentCourse)) $('#course').value = currentCourse;
  }
  ...
  function renderStudentCall(student) {
    ...
    $('#callCard').classList.toggle('d-none', !called && !student.reminderResponse);
    $('#responseGrid').classList.toggle('d-none', !called || Boolean(student.reminderResponse));
    $('#responseNote').classList.toggle('d-none', !student.reminderResponse);
    if (called) {
      $('#callMessage').textContent = `Queue ${student.queueNumber}: please confirm whether you are on the way.`;
  ...
  function renderStudentBatch(student) {
    const members = studentBatch.slice(0, CAPACITY);
    const grouped = {
  ...
  function renderFacDateTabs() {
    $('#facDateTabs').innerHTML = EVENT_DATES.map(date => `<button type="button" class="${facDate === date ? 'active' : ''}" data-date="${date}">${date === TEST_TODAY_DATE ? 'Today · ' : ''}${formatDate(date, { month: 'short', day: 'numeric' })}</button>`).join('');
  ...
  function renderFacBatchStrip() {
    ...
    $('#facBatchStrip').innerHTML = slots.map(slot => {
      const count = countFacilitatorSlot(facDate, slot.id);
      ...
      const statusText = current ? `${count} of ${CAPACITY} registered · in progress` : ended ? `${count} registered · ended` : `${count} of ${CAPACITY} registered`;
      return `<button type="button" class="batch-card ${facSlot === slot.id ? 'active' : ''} ${count >= CAPACITY ? 'full' : ''} ${current ? 'current' : ''} ${ended ? 'ended' : ''}" data-slot="${slot.id}" ${emptyEnded ? 'disabled' : ''}><span class="batch-card-top"><strong>${slot.label}</strong>${current ? '<b class="current-batch-badge">NOW</b>' : ended ? '<b class="ended-batch-badge">ENDED</b>' : ''}</span><small>${statusText}</small><span class="batch-fill" style="--batch-fill:${Math.min(100, (count / Math.max(1, CAPACITY)) * 100)}%"></span></button>`;
    }).join('');
  ...
  async function callSelectedStudent() {
    ...
    try {
      ...
      announce(`Attention ${student.firstName}. Queue number ${spellQueue(student.queueNumber)} is being called. Please respond in the Wadhwani app.`);
  ...
  function notifyStudent(student) {
    ...
    announce(`Attention ${student.firstName}. Queue number ${spellQueue(student.queueNumber)} is being called. Please respond now.`);
  }
