...
(() => {
  ...
  async function checkRegistrationIdentityLive(source = 'all', { showConflictModal = true } = {}) {
    ...
    try {
      const result = await server('checkRegistrationIdentity', fields.studentIdNumber, fields.firstName, fields.middleName, fields.lastName);
  ...
  async function handleDetailsSubmit(event) {
    ...
    try {
      const identity = await server('checkRegistrationIdentity', candidate.studentIdNumber, candidate.firstName, candidate.middleName, candidate.lastName);
