...
(() => {
  ...
  function renderStudentCall(student) {
    ...
    const message = `Queue ${student.queueNumber}: please confirm whether you are on the way.`;
  ...
  function startReminderAlarm(student) {
    ...
    reminderAlarmTimer = window.setInterval(() => playReminderAlarm(student), 1900);
    if (navigator.vibrate) {
      navigator.vibrate([400, 220, 500]);
      reminderVibrationTimer = window.setInterval(() => navigator.vibrate([400, 220, 500]), 1900);
    }
  }
  ...
  function playReminderAlarm(student) {
    try {
      ...
      [0, .22, .44].forEach((offset, index) => {
        ...
        gain.gain.exponentialRampToValueAtTime(.08, context.currentTime + offset + .02);
        gain.gain.exponentialRampToValueAtTime(.0001, context.currentTime + offset + .18);
        oscillator.start(context.currentTime + offset);
        oscillator.stop(context.currentTime + offset + .18);
      });
    } catch (_) {}

    if (soundEnabled) {
      announce(`Queue ${spellQueue(student.queueNumber)} is being called. Please choose your response.`);
    }
  }
