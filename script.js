/* Wadhwani PWA Alarm V10 — install gate + 10-minute schedule reminders */
(() => {
  'use strict';
  ...
  function handleLiveSyncVisibility() {
    ...
    if (isStudentPortalActive() && currentStudent && currentStudent.status !== 'completed') {
      setLiveSyncState('idle', document.hidden ? 'Background reminders armed' : 'Status and schedule reminders active');
  ...
  function initializePwaInstallFeature() {
    ...
    window.addEventListener('appinstalled', () => {
      ...
      if (status) status.textContent = 'Installation completed. Open Wadhwani Registration from your Home Screen or app launcher to continue.';
      updateInstallGateUi();
    });
  ...
  function updateInstallGateUi() {
    ...
    if (isInstalledPwaMode()) {
      ...
      if (status) status.textContent = 'Wadhwani is running as an installed app.';
      return;
    }
    ...
    if (deferredInstallPrompt) {
      ...
      manual.classList.add('d-none');
      if (status) status.textContent = 'Tap Install App, then approve the browser installation prompt.';
      return;
    }
    ...
    if (isIos || isSafari) {
      ...
      manual.innerHTML = '<strong>If the browser prompt is not ready:</strong><ol><li>Tap the Install button once, then wait briefly if needed.</li><li>You can also open the browser menu (usually ⋮) and choose <b>Install app</b> or <b>Add to Home screen</b>.</li><li>If Wadhwani is already installed, open it from your Home Screen or app launcher.</li></ol>';
    }
    if (status) status.textContent = 'The registration portal stays locked until it is opened in installed app mode.';
  }
  ...
  async function promptPwaInstall() {
    if (!deferredInstallPrompt) {
      updateInstallGateUi();
      const status = $('#installStatus');
      if (status) status.textContent = 'The browser install prompt is not ready yet. Wait a few seconds and tap Install again, or use the browser menu → Install app / Add to Home screen.';
      return;
    }
    const button = $('#installAppButton');
    setButtonBusy(button, true, 'Opening install prompt…');
    try {
      await deferredInstallPrompt.prompt();
      const choice = await deferredInstallPrompt.userChoice;
      if (choice?.outcome === 'accepted') {
        $('#installStatus').textContent = 'Installed. Open Wadhwani from your Home Screen or app launcher to continue.';
      } else {
        $('#installStatus').textContent = 'Installation was cancelled. Install the app to continue.';
      }
      deferredInstallPrompt = null;
      window.__wadhwaniDeferredInstallPrompt = null;
    } catch (error) {
      $('#installStatus').textContent = `Installation could not start: ${error.message}`;
    } finally {
  ...
  async function enableVolumeAlerts() {
    ...
    const extra = notificationPermission === 'granted' ? ' System notifications are also enabled.' : '';
