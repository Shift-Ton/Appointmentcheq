/* Wadhwani Remembered Student V12 — install-required PWA + FCM reminders + remembered student session */
(() => {
  'use strict';
  ...
  let deferredInstallPrompt = window.__wadhwaniDeferredInstallPrompt || null;
  ...
  let installProgressStage = 'preparing';
  ...
  async function init() {
    ...
    initializePwaInstallFeature();
    bindEvents();
    updateSoundButton();
    setPortalControls('none');
    if (!isBrowserInstructionActive() && !isInstalledPwaMode()) showInstallGate();
    else if (!isBrowserInstructionActive()) showLogin(true);
    setLoading(true, 'Reading the newest configuration and your latest registration status…', 'Connecting');
    ...
    try {
      ...
      populateColleges();
      hideOfflineModal();
      if (isInstalledPwaMode() && !isBrowserInstructionActive()) await autoRoute();
    } catch (error) {
  ...
  /* ================== PWA INSTALL + SYSTEM NOTIFICATIONS: BEGIN ================== */
  function initializePwaInstallFeature() {
    window.addEventListener('wadhwaniinstallready', () => {
      deferredInstallPrompt = window.__wadhwaniDeferredInstallPrompt || deferredInstallPrompt;
      updateInstallGateUi();
    });
    window.addEventListener('beforeinstallprompt', event => {
      event.preventDefault();
      deferredInstallPrompt = event;
      window.__wadhwaniDeferredInstallPrompt = event;
      setInstallProgress('ready', 'The browser is ready to install Wadhwani.');
      updateInstallGateUi();
    });
    window.addEventListener('appinstalled', () => {
      deferredInstallPrompt = null;
      window.__wadhwaniDeferredInstallPrompt = null;
      setInstallProgress('installed', 'Installation completed. Wadhwani is ready to open.');
      $('#openInstalledAppButton')?.classList.remove('d-none');
      const status = $('#installStatus');
      if (status) status.textContent = 'Installation completed. Tap Open Wadhwani below. If your browser does not hand the link to the installed app, open Wadhwani from your Home Screen or app launcher.';
      updateInstallGateUi();
    });
    ...
    window.setTimeout(updateInstallGateUi, 600);
  }
  ...
  function handleDisplayModeChange() {
    if (isInstalledPwaMode()) {
      hideInstallGate();
      autoRoute().catch(error => console.warn('Unable to restore remembered student:', error.message));
    } else if (!isBrowserInstructionActive()) {
      showInstallGate();
    }
  }

  function showInstallGate() {
    const gate = $('#installGateScreen');
    const app = $('#mainApplication');
    if (!gate || isBrowserInstructionActive()) return;
    app?.classList.add('d-none');
    gate.classList.remove('d-none');
    document.body.classList.add('install-gate-active');
    document.body.classList.remove('browser-check-pending');
    updateInstallGateUi();
    window.setTimeout(() => gate.focus(), 0);
  }

  function hideInstallGate() {
    $('#installGateScreen')?.classList.add('d-none');
    $('#mainApplication')?.classList.remove('d-none');
    document.body.classList.remove('install-gate-active');
  }

  function setInstallProgress(stage, message = '') {
    installProgressStage = stage;
    const states = {
      preparing: { percent: 15, label: 'Preparing app', confirm: false, ready: false },
      ready: { percent: 35, label: 'Ready for confirmation', confirm: true, ready: false },
      prompting: { percent: 55, label: 'Waiting for browser confirmation', confirm: true, ready: false },
      accepted: { percent: 82, label: 'Finishing installation', confirm: true, ready: false },
      installed: { percent: 100, label: 'Installed successfully', confirm: true, ready: true },
      manual: { percent: 35, label: 'Complete the browser install steps', confirm: true, ready: false }
    };
    const value = states[stage] || states.preparing;
    const bar = $('#installProgressBar');
    const label = $('#installProgressLabel');
    const percent = $('#installProgressPercent');
    const prepare = $('#installStepPrepare');
    const confirm = $('#installStepConfirm');
    const ready = $('#installStepReady');
    if (bar) bar.style.setProperty('--install-progress', `${value.percent}%`);
    if (label) label.textContent = value.label;
    if (percent) percent.textContent = `${value.percent}%`;
    if (prepare) prepare.classList.add('active', 'done');
    if (confirm) {
      confirm.classList.toggle('active', value.confirm && !value.ready);
      confirm.classList.toggle('done', value.confirm);
      const icon = confirm.querySelector('i');
      if (icon) icon.className = `bi ${value.confirm ? 'bi-check2-circle' : 'bi-circle'}`;
    }
    if (ready) {
      ready.classList.toggle('active', value.ready);
      ready.classList.toggle('done', value.ready);
      const icon = ready.querySelector('i');
      if (icon) icon.className = `bi ${value.ready ? 'bi-check2-circle' : 'bi-circle'}`;
    }
    if (message && $('#installStatus')) $('#installStatus').textContent = message;
  }

  function updateInstallGateUi() {
    const button = $('#installAppButton');
    const buttonText = $('#installAppButtonText');
    const openButton = $('#openInstalledAppButton');
    const manual = $('#installManualInstructions');
    const status = $('#installStatus');
    if (!button || !manual) return;

    if (isInstalledPwaMode()) {
      button.disabled = true;
      button.classList.add('d-none');
      openButton?.classList.add('d-none');
      if (buttonText) buttonText.textContent = 'App installed';
      manual.classList.add('d-none');
      setInstallProgress('installed');
      if (status) status.textContent = getRememberedStudentId() ? 'Wadhwani is installed. Your registered student profile will open automatically.' : 'Wadhwani is installed. Sign in once to remember this registered student on this device.';
      return;
    }
    ...
    if (installProgressStage === 'installed') {
      button.classList.add('d-none');
    ...
    if (deferredInstallPrompt) {
      button.disabled = false;
      if (buttonText) buttonText.textContent = 'Install Wadhwani App';
      manual.classList.add('d-none');
      if (!['prompting', 'accepted'].includes(installProgressStage)) setInstallProgress('ready');
      if (status && !['prompting', 'accepted'].includes(installProgressStage)) status.textContent = 'Tap Install App, then approve the browser installation prompt.';
      return;
    }
    ...
    setInstallProgress('manual');
    if (isIos || isSafari) {
      manual.innerHTML = '<strong>On iPhone/iPad:</strong><ol><li>Tap the <b>Share</b> button in Safari.</li><li>Choose <b>Add to Home Screen</b>.</li><li>Tap <b>Add</b>, then open Wadhwani from the new Home Screen icon.</li></ol>';
    } else {
      manual.innerHTML = '<strong>If the browser prompt is not ready:</strong><ol><li>Use the browser menu (usually ⋮).</li><li>Choose <b>Install app</b> or <b>Add to Home screen</b>.</li><li>After installation, open Wadhwani from the Home Screen/app launcher.</li></ol>';
    }
    if (status) status.textContent = 'The portal stays locked until Wadhwani is opened as an installed app.';
  }
  ...
  async function promptPwaInstall() {
    if (!deferredInstallPrompt) {
      ...
      updateInstallGateUi();
      const status = $('#installStatus');
      if (status) status.textContent = 'The browser install prompt is not available yet. Use the browser menu → Install app / Add to Home screen, or wait briefly and try again.';
      return;
    }
    const button = $('#installAppButton');
    setInstallProgress('prompting', 'Waiting for you to approve the browser installation prompt…');
    setButtonBusy(button, true, 'Confirm installation…');
    try {
      await deferredInstallPrompt.prompt();
      const choice = await deferredInstallPrompt.userChoice;
      if (choice?.outcome === 'accepted') {
        setInstallProgress('accepted', 'Installation accepted. Your browser is finishing the app installation…');
      } else {
        setInstallProgress('ready', 'Installation was cancelled. Install Wadhwani to continue.');
      }
      deferredInstallPrompt = null;
      window.__wadhwaniDeferredInstallPrompt = null;
    } catch (error) {
      setInstallProgress('ready', `Installation could not start: ${error.message}`);
    } finally {
      setButtonBusy(button, false);
      window.setTimeout(updateInstallGateUi, 350);
    }
  }
  ...
  function openInstalledApp() {
    ...
    setInstallProgress('installed', 'Opening Wadhwani…');
    const target = new URL('./?source=pwa&launch=installed', window.location.href).href;
    window.location.assign(target);
    window.setTimeout(() => {
      const status = $('#installStatus');
      if (status && !isInstalledPwaMode()) status.textContent = 'If Wadhwani stayed in the browser tab, open the installed Wadhwani icon from your Home Screen or app launcher.';
    }, 1200);
  }
  ...
  async function registerBackgroundPushForCurrentStudent(silent = true) {
    ...
    if (!isFirebasePushConfigured()) {
      if (!silent) toast('Background push not configured', 'Complete firebase-config.js and Apps Script Firebase setup to enable reminders while the app is closed.');
      return false;
    }
    if (!('Notification' in window) || Notification.permission !== 'granted') {
      if (!silent) toast('Notification permission needed', 'Allow notifications to receive reminders while Wadhwani is closed.');
      return false;
    }
    ...
    try {
      ...
      if (!silent) {
        const suffix = result?.serverConfigured === false ? ' Device registration saved, but the Apps Script FCM service account still needs configuration.' : ' Closed-app reminders are enabled for this device.';
  ...
  function bindEvents() {
    ...
    $('#installAppButton').addEventListener('click', promptPwaInstall);
    $('#openInstalledAppButton').addEventListener('click', openInstalledApp);
  ...
  function logoutStudent() {
    ...
    clearStudentSession({ forgetDevice: true });
    showLogin(true);
    toast('Logged out', 'This device will ask for your Student ID the next time Wadhwani opens.');
  }
  ...
  async function retryInternetConnection() {
    ...
    try {
      ...
      else if (isInstalledPwaMode() && getRememberedStudentId()) await autoRoute();
      else {
        await refreshPublicConfig();
        populateColleges();
      }
  ...
  async function handlePageShowFresh(event) {
    ...
    try {
      ...
      await refreshPublicConfig();
      populateColleges();
      if (isInstalledPwaMode()) await autoRoute();
      else if (!isBrowserInstructionActive()) showInstallGate();
    } catch (error) {
      if (isNetworkFailure(error)) showOfflineModal();
    }
  }
  ...
  async function autoRoute() {
    if (!isInstalledPwaMode()) {
      if (!isBrowserInstructionActive()) showInstallGate();
      return false;
    }
    ...
    try {
      const dashboard = await server('getStudentDashboard', rememberedStudentId);
      if (!dashboard?.student) {
        clearStudentSession({ forgetDevice: true });
        showLogin(true);
        toast('Registration not found', 'The saved Student ID is no longer registered. Please sign in again.');
        return false;
      }
      ...
      console.warn('Automatic student restore failed:', error.message);
      showLogin(true);
      return false;
    }
  }
  ...
  function showLogin(resetInput = false) {
    if (!isInstalledPwaMode() && !isBrowserInstructionActive()) {
      showInstallGate();
      return;
    }
    hideInstallGate();
  ...
  function showRegistration(studentIdNumber = pendingLoginId) {
    if (!isInstalledPwaMode()) { showInstallGate(); return; }
    document.body.classList.remove('fac-afternoon-theme');
  ...
  function renderStudentCompleted() {
    ...
    renderCompletedUninstallInstructions();
    clearSystemNotifications();
  }

  function renderCompletedUninstallInstructions() {
    const target = $('#completedUninstallSteps');
    if (!target) return;
    const ua = navigator.userAgent || '';
    const isIos = /iPhone|iPad|iPod/i.test(ua);
    const isAndroid = /Android/i.test(ua);
    if (isIos) {
      target.innerHTML = '<ol><li>Return to your iPhone/iPad Home Screen.</li><li>Press and hold the <strong>Wadhwani Registration</strong> icon.</li><li>Tap <strong>Remove App</strong>, then <strong>Delete App</strong> or <strong>Remove from Home Screen</strong>.</li></ol>';
    } else if (isAndroid) {
      target.innerHTML = '<ol><li>Return to your Home Screen or app drawer.</li><li>Press and hold the <strong>Wadhwani Registration</strong> icon.</li><li>Tap <strong>Uninstall</strong> or open <strong>App info</strong> → <strong>Uninstall</strong>, then confirm.</li></ol>';
    } else {
      target.innerHTML = '<ol><li>Open Wadhwani Registration.</li><li>Open the browser/app menu.</li><li>Choose <strong>Uninstall Wadhwani Registration</strong>, <strong>Remove app</strong>, or the equivalent option for your browser.</li></ol>';
    }
  }
  ...
  async function enableVolumeAlerts() {
    ...
    const extra = notificationPermission === 'granted'
      ? (backgroundReady ? ' Closed-app notifications are registered for this device.' : ' System notifications are enabled.')
      : '';
