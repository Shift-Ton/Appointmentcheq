 ...
 * 6. Set the deployed /exec URL and proxy secret in Vercel Environment Variables.
 *    The browser must call the Vercel proxy, never this web app directly.
 */
...
const API_PROXY_SETTINGS = Object.freeze({
  PROTOCOL_VERSION: 1,
  SECRET_PROPERTY: 'APPS_SCRIPT_PROXY_SECRET',
...
function doGet(e) {
  // Deliberately do not route query-string or JSONP requests. The web app URL
  // remains public for Vercel to reach it, but all direct browser calls fail.
  return jsonOutput_({
    ok: false,
    error: 'Direct API requests are not allowed.'
  });
}

function doPost(e) {
  try {
    const request = parseProxyPostRequest_(e);
    requireProxySecret_(request.proxySecret);
    const result = invokeApiAction_(request.action, request.args);
    return jsonOutput_({ ok: true, data: result });
  } catch (error) {
    console.error(error);
    return jsonOutput_({
      ok: false,
      error: error && error.message ? error.message : 'Server request failed.'
    });
  }
}

/**
 * Only functions explicitly listed here can be called through the Vercel API
 * proxy. Administrative setup functions remain editor-only.
 */
...
function parseProxyPostRequest_(event) {
  const contentType = String(event && event.postData && event.postData.type || '').toLowerCase();
  ...
  const raw = event && event.postData && typeof event.postData.contents === 'string'
    ...
    : '';
  if (!raw || raw.length > API_PROXY_SETTINGS.MAX_REQUEST_BYTES) {
    throw new Error('Invalid proxy request.');
  }
  ...
  if (!payload || typeof payload !== 'object' || Array.isArray(payload) ||
      payload.version !== API_PROXY_SETTINGS.PROTOCOL_VERSION ||
      typeof payload.action !== 'string' || !Array.isArray(payload.args) ||
      typeof payload.proxySecret !== 'string') {
    throw new Error('Invalid proxy request.');
  }

  const action = payload.action.trim();
  const proxySecret = payload.proxySecret.trim();
  if (!action || !proxySecret) throw new Error('Invalid proxy request.');
  return { action: action, args: payload.args, proxySecret: proxySecret };
}

function requireProxySecret_(providedSecret) {
  const configuredSecret = String(PropertiesService.getScriptProperties()
    .getProperty(API_PROXY_SETTINGS.SECRET_PROPERTY) || '').trim();
  if (configuredSecret.length < API_PROXY_SETTINGS.MIN_SECRET_LENGTH ||
      !safeTextEquals_(providedSecret, configuredSecret)) {
...
function setupSystem(spreadsheetId) {
  ...
  const knownDemoAccount = configRows.find(row =>
    normalizeStudentId_(row.facilitatorId) === '999-2026-000001' &&
    safeTextEquals_(row.password, 'Wadhwani123')
  );
  let initialFacilitatorCredentialChanged = false;
  if (!configRows.some(row => isValidStudentId_(row.facilitatorId))) {
    appendObject_(APP_SETTINGS.SHEETS.CONFIG, {
      ...
      name: 'Initial Facilitator',
      password: generateInitialFacilitatorPassword_(),
      active: true,
      legacyEmail: ''
    });
    initialFacilitatorCredentialChanged = true;
  } else if (knownDemoAccount) {
    knownDemoAccount.name = String(knownDemoAccount.name || 'Initial Facilitator').trim() || 'Initial Facilitator';
    knownDemoAccount.password = generateInitialFacilitatorPassword_();
    knownDemoAccount.active = true;
    updateObject_(APP_SETTINGS.SHEETS.CONFIG, knownDemoAccount._row, knownDemoAccount);
    initialFacilitatorCredentialChanged = true;
  }
  ...
  return {
    ...
    initialFacilitatorCredentialChanged: initialFacilitatorCredentialChanged,
    message: initialFacilitatorCredentialChanged
      ? 'Setup is ready. A predictable demo facilitator password was replaced with a random value; set your own strong facilitator password in the Config sheet before logging in.'
      : 'Dynamic appointment settings and background-reminder infrastructure are ready.'
  };
}

function generateInitialFacilitatorPassword_() {
