 ...
 * 4. Run setupSystem() once when installing/upgrading this schema-11 backend. It rebuilds SlotStats and installs database edit triggers.
...
function pushMessageForStudent_(student, type) {
  ...
  return {
    ...
    body: 'Attention user ' + queue + '. Your registration schedule starts in about 10 minutes' + (slot ? ' (' + slot.label + ')' : '') + '. Please prepare and proceed to the CARES Office for assistance. Thank you.',
    tag: 'wadhwani-schedule-' + String(student.date || '') + '-' + String(student.slotId || ''),
    url: './?push=schedule',
    ttl: '900'
  };
}
...
function processScheduledPushReminders() {
  ...
  registrations.forEach(student => {
    ...
    const start = slotStart_(student.date, slot);
    ...
    const reminderKey = 'schedule|' + studentId + '|' + student.date + '|' + student.slotId;
